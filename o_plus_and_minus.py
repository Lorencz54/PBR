...
def o_plus(current_frame):
    ...
    for i in range(3):
        ...
        if i == 0:
            dic_pocet_ot[int(current_frame[0])].append(e_parametr_otvoru)
            dic_var_pocet_ot[int(current_frame[0])].append(var_e_rozmer_otvoru)
        elif i == 1:
            dic_sirka_ot[int(current_frame[0])].append(e_parametr_otvoru)
            dic_var_sirka_ot[int(current_frame[0])].append(var_e_rozmer_otvoru)
        else:
            dic_vyska_ot[int(current_frame[0])].append(e_parametr_otvoru)
...
def o_minus(current_frame):
    ...
    dic_pocet_ot[int(current_frame[0])][-1].destroy()
    dic_pocet_ot[int(current_frame[0])].pop(-1)
    dic_sirka_ot[int(current_frame[0])][-1].destroy()
    dic_sirka_ot[int(current_frame[0])].pop(-1)
    dic_vyska_ot[int(current_frame[0])][-1].destroy()
    dic_vyska_ot[int(current_frame[0])].pop(-1)
    dic_typy_ot[int(current_frame[0])][-1].destroy()
    dic_typy_ot[int(current_frame[0])].pop(-1)