...
def pn(current_frame, list_l_pn, dic_pni_entries, dic_S_entries):
    pn_values = [float(entry.get()) for entry in dic_pni_entries[int(current_frame[0])]]
    S_values = [float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]]
    S_suma = np.sum((float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]))
...
def an(current_frame, list_l_an, dic_ani_entries, dic_S_entries):
    an_values = [float(entry.get()) for entry in dic_ani_entries[int(current_frame[0])]]
    S_values = [float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]]
    S_suma = np.sum((float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]))
...
def ps(current_frame, list_l_ps, dic_ps_entries, dic_S_entries):
    S_values = [float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]]
    S_suma = np.sum((float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]))
    psi_values = [float(entry.get()) for entry in dic_ps_entries[int(current_frame[0])]]
...
def hs_so(current_frame):
    pocet_values = [float(entry.get()) for entry in dic_var_pocet_ot[int(current_frame[0])]]
    sirka_values = [float(entry.get()) for entry in dic_var_sirka_ot[int(current_frame[0])]]
    vyska_values = [float(entry.get()) for entry in dic_var_vyska_ot[int(current_frame[0])]]
...
def hs(current_frame):
    S_values = [float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]]
    hsi_values = [float(entry.get()) for entry in dic_hsi_entries[int(current_frame[0])]]
    S_suma = np.sum((float(entry.get()) for entry in dic_S_entries[int(current_frame[0])]))
